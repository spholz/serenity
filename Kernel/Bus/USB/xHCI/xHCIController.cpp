...
ErrorOr<NonnullLockRefPtr<xHCIController>> xHCIController::try_to_initialize(PCI::DeviceIdentifier const& pci_device_identifier)
{
    ...
    auto pci_device = TRY(adopt_nonnull_own_or_enomem(new (nothrow) PCIDevice(pci_device_identifier)));
    auto controller = TRY(adopt_nonnull_lock_ref_or_enomem(new (nothrow) xHCIController(move(pci_device), move(registers_mapping))));
    TRY(controller->initialize());
    return controller;
}

ErrorOr<NonnullLockRefPtr<xHCIController>> xHCIController::try_to_initialize(Memory::TypedMapping<u8> registers_mapping)
{
    auto controller = TRY(adopt_nonnull_lock_ref_or_enomem(new (nothrow) xHCIController(nullptr, move(registers_mapping))));
    TRY(controller->initialize());
    return controller;
}

UNMAP_AFTER_INIT xHCIController::xHCIController(OwnPtr<PCIDevice> pci_device, Memory::TypedMapping<u8> registers_mapping)
...
void xHCIController::intel_quirk_enable_xhci_ports()
{
    ...
    SpinlockLocker const locker(m_pci_device->device_identifier().operation_lock());
    // Enable USB3 Ports
    PCI::write32_locked(m_pci_device->device_identifier(), intel_xhci_usb3_port_super_speed_enable_offset, PCI::read32_locked(m_pci_device->device_identifier(), intel_xhci_usb3_port_routing_mask_offset));
    // Enable USB2 Ports
    PCI::write32_locked(m_pci_device->device_identifier(), intel_xhci_usb2_port_routing_offset, PCI::read32_locked(m_pci_device->device_identifier(), intel_xhci_usb2_port_routing_mask_offset));
}
...
{
    if (m_pci_device)
        dmesgln_xhci("Controller found {}", PCI::get_hardware_id(m_pci_device->device_identifier()));
    else
        dmesgln_xhci("Controller found");
    ...
    if (m_pci_device) {
        PCI::enable_bus_mastering(m_pci_device->device_identifier());
        PCI::enable_memory_space(m_pci_device->device_identifier());
    }

#if ARCH(RISCV64)
    constexpr bool use_msi = false;
    dmesgln_xhci("FIXME: Disabling MSI-based interrupts on RISC-V");
#else
    constexpr bool use_msi = true;
#endif

    if (m_pci_device) {
        auto interrupt_type = TRY(m_pci_device->reserve_irqs(1, use_msi)); // TODO: Support more than one interrupter using MSI/MSI-X
        m_using_message_signalled_interrupts = interrupt_type != PCI::InterruptType::PIN;
    } else {
        m_using_message_signalled_interrupts = false;
    }

    take_exclusive_control_from_bios();

    if (m_pci_device && m_pci_device->device_identifier().hardware_id().vendor_id == PCI::VendorID::Intel)
    ...
    if (m_pci_device)
        m_interrupter = TRY(xHCIPCIInterrupter::create(*this, 0));
...
void xHCIController::handle_interrupt(Badge<xHCIPCIInterrupter>, u16 interrupter_id)
{
...
void xHCIController::event_handling_thread()
{
    while (!Process::current().is_dying()) {
        ...
        (void)m_event_queue.wait_on(timeout, "xHCI"sv);
