...
ErrorOr<void> KeyboardDriver::initialize_device(USB::Device& device, USBInterface const& interface)
{
    ...
    dbgln("keyboard: config id: {}, interface id: {}, endpoint addr: {:#x}, max packet size: {}", configuration.configuration_id(), interface.descriptor().interface_id, endpoint_descriptor.endpoint_address & 0xf, endpoint_descriptor.max_packet_size);
