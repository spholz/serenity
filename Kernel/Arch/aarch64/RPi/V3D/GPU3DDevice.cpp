...
// XXX: ioctl and mmap assume that the context stays alive during execution of them because they are passed a raw reference to the OpenFileDescription.
...
ErrorOr<GPUVirtualAddress> GPU3DDevice::allocate_buffer(Context& context, size_t size)
{
    ...
    TRY(context.region_tree.place_anywhere(*region, Memory::RandomizeVirtualAddress::No, size, V3D_PAGE_SIZE));
