...
ErrorOr<void> V3D::submit_job(PageTable const& page_table, V3DJob const& job)
{
    ...
    if ((job.tile_state_data_array_base_address % V3D_PAGE_SIZE) != 0)
        return EINVAL;
    ...
    if (binning_job_wait_result.is_error()) {
        ...
        dbgln("V3D: Attempting to halt thread 0");
    ...
    if (render_job_wait_result.is_error()) {
        ...
        dbgln("V3D: Attempting to halt thread 1");
...
ErrorOr<void> V3D::initialize()
{
    ...
    m_hub_registers->mmu_0.illegal_vaddr_target_paddr = (m_illegal_vaddr_target_page->paddr().get() >> V3D_PAGE_SHIFT) | (1u << 31);
