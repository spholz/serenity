...
static Singleton<Vector<DeviceTree::DeviceRecipe<NonnullLockRefPtr<IRQController>>>> s_recipes;
...
void InterruptManagement::find_controllers()
{
    for (auto& recipe : *s_recipes) {
        ...
        if (device_or_error.is_error()) {
            dmesgln("TimeManagement: Failed to create interrupt controller for device \"{}\" with driver {}: {}", recipe.node_name, recipe.driver_name, device_or_error.release_error());
            continue;
        }
