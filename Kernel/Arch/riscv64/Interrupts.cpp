...
void dump_registers(RegisterState const& regs)
{
    ...
    FlatPtr const sp = regs.previous_mode() == ExecutionMode::Kernel ? regs.x[1] : regs.userspace_sp();

    dbgln("ra={:p} sp={:p} gp={:p} tp={:p} fp={:p}", regs.x[0], sp, regs.x[2], regs.x[3], regs.x[7]);
