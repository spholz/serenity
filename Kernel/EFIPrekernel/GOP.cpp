...
void init_gop_and_fill_framebuffer_boot_info(BootInfo* boot_info)
{
    ...
    for (u32 mode_number = 0; mode_number < gop->mode->max_mode; mode_number++) {
        ...
        if (gop->query_mode(gop, mode_number, &size_of_mode_info, &mode_info) != EFI::Status::Success) {
            dbgln("Error: Failed to query GOP mode {}", mode_number);
            return;
        }
    ...
    if (gop->set_mode(gop, chosen_mode_number) != EFI::Status::Success) {
        dbgln("Error: Failed to set GOP mode {}", chosen_mode_number);
        return;
    }

    dbgln("GOP mode: {}x{}", chosen_mode_info.horizontal_resolution, chosen_mode_info.vertical_resolution);
