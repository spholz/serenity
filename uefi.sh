...
sgdisk -n 1::+256M -t 1:ef00 -n 2:: -t 2:8300 serenity-uefi.img
...
menuentry 'SerenityOS (normal)' {
  chainloader /Kernel.efi root=PARTUUID:${partuuid}
}

menuentry 'SerenityOS (text mode)' {
  chainloader /Kernel.efi graphics_subsystem_mode=off root=PARTUUID:${partuuid}
}

menuentry 'SerenityOS (No ACPI)' {
  chainloader /Kernel.efi root=PARTUUID:${partuuid} acpi=off
}

menuentry 'SerenityOS (with serial debug)' {
  chainloader /Kernel.efi serial_debug root=PARTUUID:${partuuid}
}

menuentry 'SerenityOS (debug tty)' {
  chainloader /Kernel.efi switch_to_tty=2 root=PARTUUID:${partuuid}
}
EOF
